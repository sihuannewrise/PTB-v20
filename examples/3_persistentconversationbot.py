...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if context.user_data:
        reply_text += (
            f" You already told me your {', '.join(context.user_data.keys())}."
            f" Why don't you "
            f"tell me something more about yourself? Or change anything "
            f"I already know."
        )
    else:
        reply_text += (
...
async def done(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        f"{facts_to_str(context.user_data)} Until next time!",
...
def main() -> None:
    ...
    application = Application.builder().token(
        TG_TOKEN).persistence(persistence).build()

    # Add conversation handler with the states CHOOSING, TYPING_CHOICE
    # and TYPING_REPLY
